    ...
    <div className="min-h-screen flex flex-col items-center justify-center p-4 animate-fade-in">
      {!selectedGender ? (
        <>
          ...
          <p className="text-gray-300 mb-8">選擇你的身份，以獲得更個人化的建議。</p>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {GENDERS.map(({ id, label, icon }) => (
              <button
                key={id}
                onClick={() => handleGenderSelect(id)}
                className="p-8 bg-gray-800 rounded-xl shadow-lg hover:shadow-violet-500/50 border border-transparent hover:border-violet-600 transition-all duration-300 transform hover:-translate-y-1"
              >
        ...
        <>
          ...
          <p className="text-gray-300 mb-8">告訴我你希望達成的關係，我會為你指引方向。</p>
          <div className="w-full max-w-2xl space-y-4">
            {GOALS.map(({ id, label, description, icon }) => (
              <button
                key={id}
                onClick={() => handleGoalSelect(id)}
                className="w-full flex items-center p-4 bg-gray-800 rounded-lg shadow-md hover:shadow-violet-500/50 border border-transparent hover:border-violet-600 transition-all duration-300 transform hover:scale-105"
              >
                <div className="text-4xl mr-4">{icon}</div>
                <div className="text-left">
                  <div className="text-lg font-bold text-gray-50">{label}</div>
